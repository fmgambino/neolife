...
export default {
  ...
  methods: {
    getData (props) {
      ...
      this.$api.get('/evangelization', { params: { page: page, limit: rowsPerPage } }).then(res => {
    ...
    saveRecord() {
      ...
      if (!this.$v.form.$error) {
        this.$q.loading.show()
        if (this.editForm) {
          this.$api.put(`/evangelization/${this.form._id}`, this.form).then(res => {
            if (res) {
              this.getData({
                pagination: this.pagination
              })
          ...
          this.$api.post('/evangelization', this.form).then(res => {
            if (res) {
              this.getData({
                pagination: this.pagination
              })
    ...
    deleteRecord(id) {
      this.$q.dialog({
        ...
        this.$api.delete(`/evangelization/${id}`).then(res => {
          if (res) {
            this.getData({
              pagination: this.pagination
            })
    ...
    saveComment () {
      ...
      if (!this.$v.comment.$error) {
        this.$q.loading.show()
        this.$api.put(`/evangelization/${this.comment.id}/comment`, { body: this.comment.body, status: this.comment.status.value }).then(res => {
          if (res) {
            this.getData({
              pagination: this.pagination
            })
    ...
    saveDestination () {
      this.$q.loading.show()
      this.$api.put(`/evangelization/${this.destinationData.data._id}`, { destination: this.destinationData.form.value }).then(res => {
        if (res) {
          this.getData({
            pagination: this.pagination
          })
